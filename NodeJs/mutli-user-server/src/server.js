    ...
    if (data.type === "login") {
      ...
      switch (data.command.toUpperCase()) {
        ...
        case "CREATE_DRONE":
          break;
        case "KILL":
          if (!data.targetEmail) throw new Error("Missing targetEmail");
          break;
        case "GET_ORDERS":
      ...
      for (const { order_id, customer_email } of deliveries) {
        ...
        if (customer) {
          customer.ws.send(
            JSON.stringify({
              type: "notification",
              message: `Order ${order_id} delivery postponed due to courier disconnection. Drone ${drone.id} has crashed.`,
            })
          );
        }
      }
...
async function handleCommand(data, ws) {
  ...
  switch (data.command?.toUpperCase()) {
    ...
    case "DRONE_STATUS":
      ...
      break;
    case "START_DELIVERY":
      if (user.userType !== "Courier") throw new Error("Permission denied");
...
async function handleGetOrders(ws, user) {
  ...
  ws.send(
    JSON.stringify({
      type: "orders",
      data: orders,
      filter: user.userType === "Courier" ? "all" : "user-specific",
    })
  );
  Logger.info({
    timestamp: new Date().toISOString(),
    ...
    context: { userType: user.userType, orderCount: orders.length, filter: user.userType === "Courier" ? "all" : "user-specific" },
  });
}
...
async function handleCreateOrder(data, ws, user) {
  ...
  ws.send(
    JSON.stringify({
      type: "command_result",
      status: "success",
      message: `Order warmcreated with ID ${order.order_id}`,
      data: order,
    })
  );
...
async function handleCurrentlyDelivering(ws, user) {
  try {
    ...
    const deliveringOrders = orders.filter((order) => order.state === "Out for delivery");
    const result = await Promise.all(
      deliveringOrders.map(async (order) => {
        const recipient = await getOrderDetails(user.apiKey, order.customer_id);
        return {
          order_id: order.order_id,
          products: order.products.map((p) => ({ title: p.title, quantity: p.quantity })),
          destination: [order.destination_latitude, order.destination_longitude],
          recipient: { name: recipient.name, surname: recipient.surname },
        };
      })
    );
    ws.send(JSON.stringify({ type: "currently_delivering", data: result }));
    Logger.info(`Currently delivering orders sent to ${user.ws.username}: ${result.length} orders`);
  } catch (error) {
...
async function handleCreateDrone(data, ws, user) {
  ...
  Logger.info(`Drone ${drone.id} created by ${user.ws.username}`);
}
...
async function handleDroneStatus(ws, user, data = {}) {
  ...
  const result = drones
    .filter((drone) => {
      ...
      if (data.current_operator_id !== undefined) return drone.current_operator_id === data.current_operator_id;
      return true;
    })
    .map((drone) => ({
      ...
      is_delivering: [...deliveryState.values()].some((d) => d.drone_id === drone.id),
    }));
  ...
  Logger.info({
    ...
    context: { droneCount: result.length, filters: data },
  });
}
...
async function handleStartDelivery(data, ws, user) {
  ...
  const drones = await getAllDrones(user.apiKey);
  const drone = drones.find((d) => d.id === parseInt(data.drone_id) && d.is_available);
  if (!drone) {
    throw new Error("Drone not found or not available");
  }
  ...
  await updateDrone(user.apiKey, drone.id, { current_operator_id: user.id });

  const recipient = await getOrderDetails(user.apiKey, order.customer_id);
  const customer_email = recipient.email || ws.username;
  await deliveryStateMutex.runExclusive(() => {
    deliveryState.set(order.order_id, { drone_id: drone.id, customer_email });
  });
...
async function updateDeliveryStatus() {
  await deliveryStateMutex.runExclusive(async () => {
    for (const [order_id, { drone_id, customer_email }] of deliveryState) {
      ...
      const speed = 10 / 111000;///will user later
      const distance =
        Math.sqrt(
